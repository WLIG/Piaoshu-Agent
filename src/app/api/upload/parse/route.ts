...
// POST /api/upload/parse - 解析上传的Word文件
    ...
    if (file.type === 'text/plain' || file.type === 'text/markdown') {
      ...
      try {
        ...
        if (file.name.endsWith('.docx')) {
          content = await parseDocxContent(buffer);
        } else {
          // 对于.doc文件，提供基本支持
          content = await parseDocContent(buffer);
        }
      } catch (error) {
        console.error('Error parsing Word file:', error);
        return NextResponse.json(
          { success: false, error: 'Failed to parse Word file. Please try converting to .txt or .md format.' },
          { status: 500 }
        );
      }
    }
...
async function parseDocxContent(buffer: Buffer): Promise<string> {
  try {
    // 这是一个简化的DOCX解析器
    // 在生产环境中，建议使用专门的库如 mammoth 或 docx-parser
...
// 解析DOC文件内容（简化版本）
async function parseDocContent(buffer: Buffer): Promise<string> {
  try {
    // DOC文件格式较复杂，这里提供基本的文本提取
    const content = buffer.toString('utf8');
    return content
      .replace(/[^\x20-\x7E\u4e00-\u9fff]/g, ' ') // 保留ASCII和中文字符
      .replace(/\s+/g, ' ')
